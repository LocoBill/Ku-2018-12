...
//	freq_turn_word = freq_word /100;
	...
	write_HMC833_24_resister(0x2, 0x2);     //分频寄存器      (不改)
	write_HMC833_24_resister(0x5, 0x0f88); 	 //
	write_HMC833_24_resister(0x5, 0xe010);   //	 Rf分频 不分频
	write_HMC833_24_resister(0x5, 0x0898);   //	1500-3000M写0x0898    分频(<1500M)写0x0A18     
	write_HMC833_24_resister(0x5, 0x1628);   //
	write_HMC833_24_resister(0x5, 0x60a0);   //
	write_HMC833_24_resister(0x5, 0x00);     // 默认 （不改）

	write_HMC833_24_resister(0x6, 0X200B4A);  //SD CFG Register  分数模式 0X200B4A   	整数模式 0X2003CA
	...
	write_HMC833_24_resister(0xc, 0xFF);         //Exact Frequency Mode Register  消除（跳动）杂散
	...
	write_HMC833_24_resister(0x3, 0x28);		    //整数寄存器 N=20   （改）分数模式N最小值20
	write_HMC833_24_resister(0x4, 0x666667);	 	 //分数寄存器
